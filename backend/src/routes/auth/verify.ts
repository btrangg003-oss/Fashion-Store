...
import { validateVerificationData } from '../services/validation'
import { 
  findTempUserByEmail, 
  deleteTempUser, 
  createUser, 
...
import { queueWelcomeEmail } from '../services/emailQueue'
import { createUserFromTempUser, checkRateLimit } from '../services/auth'
...
export default async function handler(
  ...
  try {
    ...
    // Initialize customer tier for new user
    try {
      const { initializeCustomerTier } = await import('../../../lib/autoUpdateTier');
      await initializeCustomerTier(newUser.id);
    } catch (error) {
