...
import {
  ...
  FiPlus, FiEdit2, FiTrash2, FiCopy, FiEye, FiCalendar
} from 'react-icons/fi';

const AdminMarketing = () => {
  const { user, loading } = useAuth();
  const router = useRouter();
  const [activeTab, setActiveTab] = useState<'campaigns' | 'discounts'>('campaigns');
  ...
  const [discountMode, setDiscountMode] = useState<'create' | 'edit'>('create');
  ...
  const loadData = async () => {
    try {
      const [campaignsRes, discountsRes] = await Promise.all([
      ...
      <Container>
        ...
        <StatsGrid>
          ...
          <StatCard>
            ...
            <StatContent>
              ...
              <StatValue>{stats.activeDiscounts}/{stats.totalDiscounts}</StatValue>
        ...
        <TabsContainer>
          <Tab active={activeTab === 'campaigns'} onClick={() => setActiveTab('campaigns')}>
            <FiTarget /> Chiến dịch ({campaigns.length})
          </Tab>
          <Tab active={activeTab === 'discounts'} onClick={() => setActiveTab('discounts')}>
            <FiPercent /> Mã giảm giá ({discounts.length})
          </Tab>
        </TabsContainer>
