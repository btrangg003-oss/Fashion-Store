...
import { FiPlus, FiSearch, FiEye, FiEdit, FiTrash2, FiDownload, FiPrinter, FiFileText, FiMoreVertical } from 'react-icons/fi';
...
const InboundListPage = () => {
  ...
  const handleEdit = (id: string) => {
    router.push(`/admin/inventory/inbound/create?id=${id}`);
  };
  ...
  const handlePrintBarcode = (movement: any) => {
    // Generate professional shipping label like Shopee Express
    ...
    if (barcodeWindow) {
      ...
      barcodeWindow.document.write(`
        ...
        <html>
        <head>
          <title>Phiếu Nhập Kho - ${movement.receiptNumber}</title>
          <meta charset="UTF-8">
        ...
        <body>
          ...
          <div class="label">
            ...
            <div class="barcode-section">
              ...
              <div class="barcode-label" style="margin-top: 10px;">Mã đơn hàng: ${movement.invoiceNumber || movement.receiptNumber}</div>
            </div>
            ...
            <div class="info-grid">
              ...
              <div class="info-box">
                ...
                <div class="info-text">
                  ...
                  ${movement.supplierName || 'Nhà Cung Cấp'}<br>
                  ${movement.notes || 'Địa chỉ nhà cung cấp'}<br>
                  SĐT: ${movement.supplierPhone || '-'}
                </div>
              </div>
            </div>
            ...
            <div class="qr-section">
              ...
              <div class="weight-section">
                <div class="weight-label">Khối lượng tối đa:</div>
            ...
            <!-- Footer -->
            <div class="footer">
              Tuyển dụng Tài xế/Điều phối kho SPX - Thu nhập 8-20 triệu - Gọi 1900 6885
            </div>
          </div>
    ...
    if (printWindow) {
      printWindow.document.write(`
        ...
        <body>
          ...
          <div class="info">
            <p><strong>Ngày nhập:</strong> ${new Date(movement.receiptDate).toLocaleDateString('vi-VN')}</p>
            <p><strong>Nhà cung cấp:</strong> ${movement.supplierName || '-'}</p>
            <p><strong>Kho nhập:</strong> ${movement.warehouseName || 'Kho chính'}</p>
            <p><strong>Người tạo:</strong> ${movement.createdBy || 'Admin'}</p>
          </div>
          <table>
            <thead>
              <tr>
  ...
  const handleDownloadReceipt = (movement: any) => {
    ...
    // Create CSV content
    let csvContent = `Phiếu Nhập Kho\n`;
    csvContent += `Mã phiếu:,${movement.receiptNumber}\n`;
    csvContent += `Ngày nhập:,${new Date(movement.receiptDate).toLocaleDateString('vi-VN')}\n`;
    csvContent += `Nhà cung cấp:,${movement.supplierName || '-'}\n`;
    csvContent += `Kho nhập:,${movement.warehouseName || 'Kho chính'}\n\n`;
    ...
    link.download = `Phieu_Nhap_${movement.receiptNumber}_${Date.now()}.csv`;
    link.click();
  };

  const handlePrint = (id: string, type: 'barcode' | 'qr' | 'both') => {
    alert(`In ${type === 'barcode' ? 'Mã vạch' : type === 'qr' ? 'QR Code' : 'Cả 2'} cho phiếu ${id}`);
    setShowPrintMenu(null);
  };

  const handleDownloadPDF = (id: string) => {
    alert(`Tải file PDF cho phiếu ${id}`);
  };
                      ...
                      <tr key={`${movement.id}-${itemIndex}`}>
                        {itemIndex === 0 && (
                          <>
                            ...
                            <Td rowSpan={movement.items.length}>{movement.supplierName || '-'}</Td>
                        ...
                        <Td>{item.unit || 'Cái'}</Td>
                        <Td><Price>{(item.costPrice || item.price || 0).toLocaleString()}₫</Price></Td>
                        {itemIndex === 0 && (
                          <>
                            <Td rowSpan={movement.items.length}>
